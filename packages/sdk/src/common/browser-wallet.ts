...
export class BrowserWallet extends AccountUtils {
    wallet: SignerWalletAdapter; // browser wallet, used in non-cli env (aka prod)

    constructor(connection: Connection, wallet: SignerWalletAdapter) {
        super(connection);
        this.wallet = wallet;
    }
    ...
    sendAndConfirmTx = async (tx: TxWithSigners): Promise<string> => {
        //these need to be done manually for a raw tx
        tx.tx.recentBlockhash = (
        ...
        return sendAndConfirmRawTransaction(this.connection, tx.tx.serialize());
    }

    sendTxWithWallet = async (tx: TxWithSigners) => {
        tx.signers.forEach((s) => {
            tx.tx.partialSign(s);
        });
        return this.wallet.sendTransaction(tx.tx, this.connection);
    }

    // ----------------- multiple

    mergeTxs = async (txs: TxWithSigners[]): Promise<TxWithSigners> => {
    ...
    sendAndConfirmTxsSet = async (txs: TxWithSigners[]): Promise<string[]> => {
        console.log(`attempting to send ${txs.length} transactions`);
        const signatures = await Promise.all(
            txs.map((t) => this.sendAndConfirmTx(t))
        );
        const result = await Promise.all(
            signatures.map((s) => this.connection.confirmTransaction(s))
        );
    ...
    sendTxsSetWithWallet = async (txs: TxWithSigners[]) => {
